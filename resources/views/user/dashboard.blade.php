...
@if($application)
    ...
    <div style="background:linear-gradient(135deg,#FFFBEB,#FEF3C7);border:1.5px solid #F59E0B;border-radius:14px;padding:18px 22px;margin-bottom:24px;display:flex;align-items:center;gap:14px;">
        ...
        <div style="flex:1;">
            <div style="font-weight:700;font-size:14px;color:#92400E;">{{ __('Agent Application Under Review') }}</div>
        ...
        <span style="padding:6px 14px;background:#F59E0B;color:white;border-radius:9999px;font-size:12px;font-weight:700;white-space:nowrap;">{{ __('Pending Review') }}</span>
    </div>
    ...
    <div style="background:#FEF2F2;border:1.5px solid #FECACA;border-radius:14px;padding:18px 22px;margin-bottom:24px;display:flex;align-items:flex-start;gap:14px;">
        ...
        <div style="flex:1;">
            <div style="font-weight:700;font-size:14px;color:#991B1B;">{{ __('Agent Application Not Approved') }}</div>
            <div style="font-size:13px;color:#7F1D1D;margin-top:4px;"><strong>{{ __('Reason:') }}</strong> {{ $application->admin_notes }}</div>
            <a href="{{ route('become.agent') }}" style="display:inline-flex;align-items:center;gap:6px;margin-top:10px;padding:8px 16px;background:#DC2626;color:white;border-radius:9999px;font-size:13px;font-weight:700;text-decoration:none;">
                <i class="fa-solid fa-rotate-right"></i> {{ __('Resubmit Application') }}
            </a>
        </div>
    </div>
    ...
    <div style="background:linear-gradient(135deg,#ECFDF5,#D1FAE5);border:1.5px solid #6EE7B7;border-radius:14px;padding:18px 22px;margin-bottom:24px;display:flex;align-items:center;gap:14px;">
        ...
        <div style="flex:1;">
            <div style="font-weight:700;font-size:14px;color:#065F46;">{{ __('Your agent application is approved!') }}</div>
            <div style="font-size:13px;color:#047857;">{{ __('Redirecting you to Agent Dashboard…') }}</div>
        </div>
        <a href="{{ route('agent.dashboard') }}" style="padding:8px 18px;background:#059669;color:white;border-radius:9999px;font-size:13px;font-weight:700;text-decoration:none;">{{ __('Go to Agent Dashboard →') }}</a>
    </div>
    @endif
@endif
...
<div class="stats-grid" style="margin-bottom:28px;">
    ...
    <div class="stat-card">
        ...
        <div class="stat-info">
            ...
            <div class="stat-label">{{ __('New Listings') }}</div>
        </div>
    </div>
</div>
...
<div style="display:grid;grid-template-columns:1fr 1fr;gap:24px;">
    ...
    <div class="card">
        <div class="card-header">
            <div class="card-title"><i class="fa-solid fa-calendar" style="color:var(--primary);"></i> {{ __('My Recent Bookings') }}</div>
            <a href="{{ route('appointments.index') }}" class="card-action">{{ __('View All →') }}</a>
        </div>
        ...
        <div class="card-empty" style="padding:36px 20px;">
            ...
            <a href="{{ route('listings.index') }}" class="btn-primary btn-sm">{{ __('Browse Properties') }}</a>
        </div>
        @endforelse
    </div>
    ...
    <div style="display:flex;flex-direction:column;gap:20px;">
        ...
        <div style="background:linear-gradient(135deg,var(--primary),#2D6A4F);border-radius:16px;padding:24px;color:white;position:relative;overflow:hidden;">
            ...
            <div style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:1px;opacity:0.7;margin-bottom:8px;">{{ __('Career Opportunity') }}</div>
            <div style="font-size:20px;font-weight:800;margin-bottom:8px;line-height:1.2;">{{ __('Become a Verified') }}<br>{{ __('NyumbaHub Agent') }}</div>
            <div style="font-size:13px;opacity:0.85;margin-bottom:18px;">{{ __('List properties, earn commissions, and grow your real estate business on our platform.') }}</div>
            <a href="{{ route('become.agent') }}"
                ...
                <i class="fa-solid fa-briefcase"></i> {{ __('Apply Now') }}
            </a>
        </div>
        @endif
        ...
        <div class="card" style="flex:1;">
            <div class="card-header">
                <div class="card-title"><i class="fa-solid fa-building" style="color:var(--primary);"></i> {{ __('New Listings') }}</div>
                <a href="{{ route('listings.index') }}" class="card-action">{{ __('Browse All →') }}</a>
            </div>
