...
<div style="max-width:640px;margin:0 auto;text-align:center;padding:60px 20px;">
    ...
    <h2 style="font-size:26px;font-weight:800;color:var(--gray-900);margin-bottom:10px;">You're a Verified Agent</h2>
    <p style="font-size:15px;color:var(--gray-500);margin-bottom:28px;">You already have full access to the Agent Dashboard, listings management, and appointment tools.</p>
    <a href="{{ route('agent.dashboard') }}" class="btn-primary" style="padding:12px 32px;font-size:15px;">
        <i class="fa-solid fa-gauge"></i> Go to Agent Dashboard
    </a>
</div>
...
<div style="max-width:640px;margin:0 auto;">
    <div style="background:linear-gradient(135deg,#FFFBEB,#FEF3C7);border:2px solid #F59E0B;border-radius:20px;padding:36px;text-align:center;margin-bottom:24px;">
        ...
        <h2 style="font-size:22px;font-weight:800;color:#92400E;margin-bottom:8px;">Application Under Review</h2>
        <p style="font-size:14px;color:#B45309;margin-bottom:16px;">Your application was submitted on <strong>{{ $application->created_at->format('d M Y \a\t H:i') }}</strong>. Our team typically reviews applications within 24–48 hours.</p>
    ...
    <div class="card">
        ...
        <div style="padding:0 20px 20px;">
            <div style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:0.5px;color:var(--gray-400);margin-bottom:8px;">Profile Photo</div>
...
<div style="background:#FEF2F2;border:1.5px solid #FECACA;border-radius:14px;padding:18px 22px;margin-bottom:28px;max-width:760px;">
    <div style="display:flex;align-items:flex-start;gap:14px;">
        ...
        <div>
            <div style="font-weight:700;font-size:14px;color:#991B1B;margin-bottom:4px;">Previous Application Rejected</div>
            <div style="font-size:13px;color:#7F1D1D;"><strong>Reason:</strong> {{ $application->admin_notes }}</div>
            <div style="font-size:12px;color:#B91C1C;margin-top:6px;">Please address the feedback and submit a new application below.</div>
        </div>
    </div>
</div>
@endif
...
<div style="display:grid;grid-template-columns:1fr 320px;gap:28px;max-width:1100px;align-items:start;">
    ...
    <div class="card">
        <div style="padding:28px 28px 0;">
            <h2 style="font-size:20px;font-weight:800;color:var(--gray-900);margin-bottom:4px;">Agent Application Form</h2>
            <p style="font-size:14px;color:var(--gray-500);margin-bottom:0;">Complete all required fields. We'll review your application and respond within 24–48 hours.</p>
        </div>
        ...
        <form method="POST" action="{{ route('become.agent.store') }}" enctype="multipart/form-data" style="padding:24px 28px 28px;">
            ...
            <div class="field-row">
                ...
                <div class="field">
                    <label for="nida_number">NIDA / National ID Number <span class="required">*</span></label>
                    <input type="text" id="nida_number" name="nida_number"
                        value="{{ old('nida_number') }}"
                        placeholder="e.g. 19870101-12345-00001-7"
            ...
            <div class="field-row">
                <div class="field">
                    <label for="agency_name">Agency Name <span style="color:var(--gray-400);font-weight:400;">(Optional)</span></label>
                    <input type="text" id="agency_name" name="agency_name"
                        value="{{ old('agency_name') }}"
                        placeholder="e.g. Kilimanjaro Realty"
            ...
            <div class="field">
                <label for="bio">Professional Biography <span class="required">*</span></label>
                <textarea id="bio" name="bio" rows="5"
                    placeholder="Describe your real estate experience, specialties, areas you cover, and why you want to join NyumbaHub. Minimum 50 characters."
                    class="{{ $errors->has('bio') ? 'is-invalid' : '' }}" required>{{ old('bio') }}</textarea>
                <div style="font-size:12px;color:var(--gray-400);margin-top:4px;">Minimum 50 characters. Be specific about your experience and expertise.</div>
            ...
            <div class="field-row">
                <div class="field">
                    <label for="profile_photo">Profile Photo <span style="color:var(--gray-400);font-weight:400;">(Optional)</span></label>
                    <input type="file" id="profile_photo" name="profile_photo"
                        accept="image/jpeg,image/jpg,image/png,image/webp"
                        class="{{ $errors->has('profile_photo') ? 'is-invalid' : '' }}"
                        style="padding:8px;">
                    <div style="font-size:12px;color:var(--gray-400);margin-top:4px;">JPG, PNG or WebP · Max 3MB</div>
                    @error('profile_photo') <div class="field-error">{{ $message }}</div> @enderror
                </div>
                <div class="field">
                    <label for="supporting_document">Supporting Document <span style="color:var(--gray-400);font-weight:400;">(Optional)</span></label>
                    <input type="file" id="supporting_document" name="supporting_document"
                        accept=".pdf,image/jpeg,image/jpg,image/png"
                        class="{{ $errors->has('supporting_document') ? 'is-invalid' : '' }}"
                        style="padding:8px;">
                    <div style="font-size:12px;color:var(--gray-400);margin-top:4px;">License, certificate or ID copy · PDF or image · Max 5MB</div>
    ...
    <div style="display:flex;flex-direction:column;gap:18px;position:sticky;top:24px;">
        <div style="background:linear-gradient(135deg,var(--primary),#2D6A4F);border-radius:16px;padding:24px;color:white;">
            <div style="font-size:16px;font-weight:800;margin-bottom:14px;">Why become an Agent?</div>
            @foreach([
        ...
        <div class="card">
            <div style="padding:20px;">
                <div style="font-size:14px;font-weight:700;color:var(--gray-900);margin-bottom:10px;"><i class="fa-solid fa-circle-info" style="color:var(--primary);"></i> Review Process</div>
                <ol style="font-size:13px;color:var(--gray-600);padding-left:18px;line-height:1.8;">
                    <li>Submit your application</li>
                    <li>Admin reviews within 24–48 hrs</li>
                    <li>Receive email notification</li>
                    <li>Access Agent Dashboard immediately upon approval</li>
                </ol>
            </div>
        </div>
    </div>
